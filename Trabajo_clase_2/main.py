...
class ObtenerDatos:
    ...
    def load_data_from_files(self):
        all_data_frames = [] # Lista vacia para concatenar los datos de todos los archivos por set de datos.
        ...
        file_names = (f for f in scandir(self.data_folder)) # Almaceno nombres de archivos por carpeta de set de datos.
        
        # El sigueinte for recorre cada uno de los generadores con los nombres de archivos por set de datos
        for file_name in file_names:
            file_path = path.join(self.data_folder, file_name.name) # Obtengo propiamente el nombre del archivo con .name y lo concateno a la ruta de la carpeta
            # Lee el archivo en chunks y selecciona las columnas necesarias que van desde 0 hasta self.channels, los dtos se encuentran separados por espacios (\t)
            file_data = pd.concat(
            ...
            all_data_frames.append(file_data) # Almaceno en all_data_frames los datos de cada uno de los archivos leidos.
...
set_2.reduce_dimensionality(data_2, 2) # Se aplica PCA para los datos del set #2 y para 2 componentes principales
